...
API.prototype.publishTxProposal = function(opts, cb) {
  ...
  $.checkState(parseInt(opts.txp.version) >= 1);

  var self = this;
  console.log('API.prototype.publishTxProposal txp', opts.txp);

  var t = Utils.buildTx(opts.txp);
  console.log('API.prototype.publishTxProposal t', t);

  var hash = t.uncheckedSerialize();
  console.log('API.prototype.publishTxProposal hash', hash);
  var args = {
    proposalSignature: Utils.signMessage(hash, self.credentials.requestPrivKey)
  };

  console.log('API.prototype.publishTxProposal proposalSignature', args.proposalSignature);
